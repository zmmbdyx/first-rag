...
**结构感知的智能切分**、**向量 + BM25 关键词混合检索（RRF 融合）**、**CrossEncoder 重排**、**段落级答案溯源**、**多轮对话查询改写**，
并附带一套 **244 题评测集与可复现的评测框架**（bootstrap 置信区间、忠实度/相关性指标、逐模块消融与显著性检验）。

**当前效果（v2 评测集，244 题 / 31 篇语料 / 207 道可回答题）**：

| 指标 | 值 |
|---|---|
| 检索 Recall@1 / @3 / @5 | **88.4% / 98.6% / 99.0%**（开启重排） |
| 检索 MRR | 0.931 |
| 端到端准确率 | **96.6%** [95% CI 94.2%, 99.0%] |
| 忠实度 / 引用率 | 0.952 / 99.5% |

> **不要只看 Recall@5**：R@1 为 88.4%，意味着约 12% 的问题首位召回是错的，只是被 top-5 的冗余兜住；
> 判断检索质量应以 R@1/MRR 为准。另：**关闭重排时 R@1 只有 66.2%、R@5 95.7%**，代价与取舍见第 5.3 节。
...
| 📊 效果评测 | 31 篇语料 / **244 题**（可回答 207 + 拒答 37；其中单轮 156 + 多轮 16 + 多跳 15 + 同义改写 8 + 对抗错误前提 12），五项指标（Recall/MRR/准确率/忠实度/相关性），95% bootstrap 置信区间、按文档类型与题型分项、配对显著性检验、RRF 网格搜索与 BM25 变体消融、Reranker 开关对比 |
...
本项目有两套评测集，**结论以大规模的 v2 为准**：

| 评测集 | 题量 | 语料 | 用途 |
|---|---|---|---|
| `eval/questions.jsonl`（v1） | 30 题（26 可回答 + 4 拒答） | `data/samples` 4 篇 | 早期快速回归，**样本太小，只作方向性参考** |
| `eval/questions_v2.jsonl`（v2） | **244 题**（207 可回答 + 37 拒答） | `data/corpus` **31 篇** | 正式评测：含 multi_hop 15 / multi_turn 16 / paraphrase 8 / adversarial 12，带 bootstrap 95% CI |

- **指标**：检索看 Recall@k / MRR。命中判定是**答案级**的——金标答案取自文档原文片段（按 `；` 切分），
  **同一个召回块必须包含全部分片**才算命中，且文档名必须匹配，因此不会"召回同一篇文档就算对"；
- **回答质量**：LLM-as-Judge 三档（correct/partial/wrong）+ 忠实度 + 相关性；
- **消融**：固定窗口切分（naive）→ 智能切分 → +混合检索 → +重排，逐层隔离贡献。

> ⚠️ **重排必须显式标注**。`CONFIGS` 里每个配置现在都写死了 `rerank` 开关，并写入 `_meta.rerank_per_config`。
> 此前该键缺失，`Retriever.retrieve(rerank_on=None)` 会回落到 `RERANK_ENABLED`（默认 `auto`），
> 于是"混合检索"实际是在**开启重排**下测的却未标注 —— 这正是下面新旧口径差异的来源。

### 5.2 结果（v2，207 道可回答题）

**检索效果**（本次已显式区分重排开关，结果见 `eval/results/eval_results_retrieval.json`）

| 配置 | Recall@1 | Recall@3 | Recall@5 | MRR |
|---|---|---|---|---|
| 基线：固定窗口切分 + 纯向量 | 0.517 | 0.744 | 0.812 | 0.632 |
| 智能切分 + 纯向量 | 0.575 | 0.778 | 0.812 | 0.672 |
| 智能切分 + 混合检索（**关闭重排**） | 0.662 | 0.894 | 0.957 | 0.780 |
| 智能切分 + 混合检索（**开启重排**，即默认配置） | **0.884** | **0.986** | **0.990** | **0.931** |

**端到端回答质量**（开启重排，244 题，`eval/results/report_v2.md`）

| 指标 | 值 [95% CI] |
|---|---|
| 准确率（correct） | 96.6% [94.2%, 99.0%] |
| 忠实度 | 0.952 [0.927, 0.973] |
| 相关性 | 0.870 |
| 引用率 / 溯源准确率 | 99.5% / 97.1% |
| 拒答正确率（37 题） | 89.2% [78.4%, 97.3%] |

**分题型 Recall@5 / 准确率**：single 100%/97%（156 题）、multi_hop 100%/100%（15）、adversarial 100%/100%（12）、
paraphrase 100%/88%（8）、multi_turn 94%/88%（16）；**表格类文档是明确短板**（17 题，准确率 71%）。

### 5.3 结论与诚实说明

**这一节的结论在换成 v2 大题集后被修正过，务必看新版：**

- ❌ **旧结论（基于 v1 26 题）"智能切分是最大收益来源"不成立**。v1 上 Recall@5 从 88.5%→100%，看似切分功劳巨大；
  但在 v2 的 207 题上，智能切分对 Recall@5 的增益是 **0**（0.812→0.812），只在 R@1/MRR 上有小幅提升
  （0.517→0.575 / 0.632→0.672）。26 题的样本量根本支撑不起"某模块带来 +11.5pp"这种结论。
- ✅ **真正的收益来自混合检索与重排**：混合检索把 Recall@5 从 0.812 拉到 0.957（+14.5pp），
  重排再把 R@1 从 0.662 拉到 0.884（+22.2pp）、MRR 从 0.780 到 0.931。
  也就是说"关键词路兜底 + CrossEncoder 精排"才是排序质量的主要来源。
- ⚠️ **重排的代价是延迟**：实测每题约 **29ms → 3570ms**（`eval/results/rerank_compare.md`），
  换来 R@1 +10.5pp、R@5 +2.1pp。对延迟敏感的场景应当关闭重排（此时 R@5 仍有 95.7%）。
- ⚠️ **`R@5≈99%` 不是"检索已经解决"**：R@1 只有 0.88，说明**约 12% 的问题首位召回是错的**，
  只是靠 top-5 的冗余兜住了。真实价值应看 R@1/MRR，而不是只报 R@5。
- ⚠️ **重排引入运行间波动**：同一配置两次评测得到 R@5=0.9952 与 0.9903、R@1=0.8599 与 0.8841，
  差异大于 0（非确定性来自 CrossEncoder 批量推理与索引重建），因此**不宜把 0.5pp 级别的差异当结论**。
- ⚠️ **评测语料是合成的、且规模小**（31 篇）。`scripts/benchmark_scale.py` 使用的 1000 篇
  放大语料是**同一句式模板只替换数字**生成的（每篇 700 字节左右），在那上面测召回会显著虚高，
  不能代表真实企业语料。同理金标答案与问题同源，题目难度低于真实用户提问。
- ⚠️ **LLM-as-Judge 与被测模型同源**，存在自判偏差；拒答题 37 题、多轮 16 题的 CI 较宽。

> 复现：`python scripts/run_eval.py --retrieval-only`（秒级，不需要 API Key，输出带重排开关标注的对照表）。
> 校验评测集本身是否"太容易"：`python scripts/audit_eval_validity.py`。
...
python scripts/run_eval.py --legacy # v1 30 题全量评测（注意：样本小，仅作回归）

# 正式评测（v2：31 篇语料 / 244 题，含置信区间与消融）
python scripts/make_corpus.py # 生成语料与评测集
python scripts/run_eval.py --retrieval-only # 仅检索指标（秒级、无需 API Key）
python scripts/run_eval.py # 全配置全量评测（五项指标 + CI，需 API Key）
...
# 结果输出至 eval/results/（report_v2.md / eval_results_retrieval.json / tuning.md / benchmark_*.md）
```
...
│   ├── make_corpus.py # 生成 v2 语料（31 篇）与评测集（244 题）
...
│   ├── questions_v2.jsonl # v2 评测集（244 题：207 可回答 + 37 拒答）
│   └── results/ # 全部评测报告与明细（已提交，可直接查看）
├── data/
│   ├── samples/ # v1 示例文档
│   ├── corpus/ # v2 语料（31 篇：制度/合同/说明书/表格/FAQ/技术文档）
