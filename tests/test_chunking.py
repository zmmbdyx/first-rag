...
def _fake_doc() -> ParsedDoc:
    long_para = "这是第一个句子。这是第二个句子。" * 40  ***REMOVED*** ~640 字，超过 chunk_size
...
def test_smart_chunk_respects_sections():
    ...
    assert len(chunks) >= 4
    ***REMOVED*** 不允许跨章节：第一章的块不能混入第二章标题之后的文本
...
def test_naive_chunk_covers_all_text():
    ...
    assert chunks[-1].text.endswith(text[-50:])  ***REMOVED*** 尾部完整覆盖
    assert len(chunks) == math.ceil((1000 - 200) / 150) + 1  ***REMOVED*** 首块 + 滑窗次数
