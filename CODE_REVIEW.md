...
***REMOVED******REMOVED*** 【严重问题】（必须修复）

***REMOVED******REMOVED******REMOVED*** 🔴 1. `metrics.record()` 只捕获 OSError，SQLite 异常会炸穿 chat 主流程
...
- **修复**：
```python
    except sqlite3.Error as e:      ***REMOVED*** 指标失败绝不影响主流程
        logging.warning("metrics 落库失败: %s", e)
```
- 同类问题：`_check_alerts()`→`aggregate()` 里的 DB 读取同在此 try 内，一并覆盖。

***REMOVED******REMOVED******REMOVED*** 🔴 2. 网页上传的文件名未消毒，存在路径穿越（Path Traversal）
...
```python
        for f in uploaded:
            safe_name = Path(f.name).name                      ***REMOVED*** 去掉任何目录成分
...
***REMOVED******REMOVED******REMOVED*** 🔴 3. OpenAI 客户端没有超时与重试上限
...
***REMOVED******REMOVED******REMOVED*** 🟡 4. `_last_usage` 模块级全局变量存在并发竞态与归因错误
...
def chat(messages, model=..., ...) -> str:
    ...
    _LAST_USAGE.local = resp.usage   ***REMOVED*** threading.local()
***REMOVED*** 或更彻底：chat 返回 (text, usage)，调用方按需取用
```

***REMOVED******REMOVED******REMOVED*** 🟡 5. BM25 路径存在 N+1 查询
...
***REMOVED******REMOVED******REMOVED*** 🟡 6. `st.session_state` 在 `@st.cache_resource` 函数内赋值，仅对首个会话生效
...
***REMOVED******REMOVED******REMOVED*** 🟡 7. 超长输入静默截断，无提示
...
***REMOVED******REMOVED*** 【优先行动清单 Top 5】（影响面 × 修复成本）
