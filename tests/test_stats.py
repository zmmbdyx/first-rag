...
def test_bootstrap_ci_contains_mean():
    vals = [1.0] * 70 + [0.0] * 30 # 均值 0.7
    mean, lo, hi = bootstrap_ci(vals, n_boot=500)
    assert abs(mean - 0.7) < 1e-9
    assert lo <= mean <= hi
    assert 0.55 < lo and hi < 0.85 # 200 样本的 CI 不应过宽


def test_paired_diff_detects_improvement():
    a = [0.0] * 60 + [1.0] * 40 # 基线 40%
    b = [0.0] * 30 + [1.0] * 70 # 改进后 70%
    r = paired_bootstrap_diff(b, a) # B - A
    assert r["diff"] > 0.25
    assert r["lo"] > 0 # 显著为正
    assert r["p"] < 0.05
    assert r["n"] == 100
